    ...
    <div className='bg-half-transparent w-screen fixed nav-item top-0 right-0'>
      <div className='float-right h-screen dark:text-gray-200 bg-white dark:[#484b52] w-400'>
        <div className='flex justify-between items-center p-4 ml-4'>
          ...
          <button type='button' className='text-2xl p-3 hover:drop-shadow-xl hover:bg-light-gray' onClick={() => setThemeSettings(false)} style={{color: 'rgb(153,171,180)', borderRadius: '50%'}}><MdOutlineCancel/></button>
        </div>
        <div className='flex-col border-t-1 border-color p-4 ml-4'>
          <p className='font-semibold text-lg'>Theme Options</p>
          <div className='mt-4'>
            <input className='cursor-pointer' type='radio' id='light' name='theme' checked={currentMode === 'Light'} value='Light' onChange={setMode}/>
            <label htmlFor='light' className='ml-2 text-md cursor-pointer'>Light</label>
          </div>
          <div className='mt-4'>
            <input className='cursor-pointer' value='Dark' type='radio' id='dark' name='theme' checked={currentMode === 'Dark'} onChange={setMode}/>
